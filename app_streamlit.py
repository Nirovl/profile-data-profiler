...
# === НАСТРОЙКИ ===
RULES_OPTIONS = {
    "Analytics": "analytics_rules.json",
    "Banking": "banking_rules.json",
    "Education": "education_rules.json",
    "Finance": "finance_rules.json",
    "HR": "hr_rules.json",
    "Inventory": "inventory_rules.json",
    "Marketing": "marketing_rules.json",
    "Sales": "rules.json",
}
UPLOADS_DIR = Path("uploads")

st.set_page_config(
    page_title="Профайлер качества данных",
    page_icon="📊",
    layout="wide"
)

st.markdown("<h1 style='color: #102A43; font-size: 38px; line-height: 1.1;'>📊 Узнайте, насколько качественны ваши данные, прежде чем строить на них отчеты и принимать бизнес-решения.</h1>", unsafe_allow_html=True)
st.markdown("<p style='color: #4A4A4A; font-size: 18px; line-height: 1.6; max-width: 900px;'>Data Quality Profiler — инструмент для автоматического тестирования и профилирования данных, который за считанные секунды оценит их по ключевым критериям: валидность, точность, согласованность, объективность и разумность, а также выявит все пропуски, дубли и аномалии.</p>", unsafe_allow_html=True)
st.markdown("---")

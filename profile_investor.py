...
def load_rules(rules_file):
    if not os.path.exists(rules_file):
        raise FileNotFoundError(f'Не найден файл правил: {rules_file}')
...
def parse_args():
    ...
    parser.add_argument('rules_file', nargs='?', default=DEFAULT_RULES_FILE, help='JSON-файл с правилами')
...
def get_report_title(data_file):
    name = os.path.basename(data_file)
    domain = ''
    if name == 'sales_data_sample.csv':
        domain = ' (Sales)'
    elif name == 'hr_data_sample.csv':
        domain = ' (HR)'
    elif name == 'inventory_data_sample.csv':
        domain = ' (Inventory)'
    return f'# 📊 Инвестиционный отчёт о качестве данных{domain}'
...
report.append(f'**Правила:** {RULES_FILE}')
report.append('')

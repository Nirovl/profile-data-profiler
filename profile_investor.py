...
if __name__ == '__main__':
    # shim for backward compatibility: call new profile.py with same args
    new = Path('profile.py')
    if not new.exists():
        print('Error: profile.py not found', file=sys.stderr)
        sys.exit(1)
    args = [str(new)] + sys.argv[1:]
    os_cmd = [sys.executable] + args
    import subprocess
    completed = subprocess.run(os_cmd)
